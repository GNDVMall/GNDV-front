<template>
  <form class="my-10">
    ...
    <Button
      :text="'등록'"
      :type="'green'"
      :click-handler="handleFormButton"
    ></Button>
  </form>
</template>
...
const fetchData = async () => {
  ...
  let tradeOpts = [];
  if (res.data.data.product_trade_opt1 === "Y")
    tradeOpts.push("product_trade_opt1");
  if (res.data.data.product_trade_opt2 === "Y")
    tradeOpts.push("product_trade_opt2");
...
const handleFormButton = async (e) => {
  ...
  request.append(
    "product_trade_opt1",
    formData.value.tradeOpt.includes("product_trade_opt1") ? "Y" : "N"
  );
  request.append(
    "product_trade_opt2",
  ...
  if (props.type === "EDIT") {
    ...
    const deleted = prevImages.value.filter(
